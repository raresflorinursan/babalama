...
export function ProjectCard({ project, index = 0 }: { project: ProjectCardData; index?: number }) {
  return (
    <TiltCard delay={Math.min(index, 8) * 60} className="rounded-xl">
      <Link
        to="/projects/$id"
        params={{ id: project.id }}
        className="group flex h-full flex-col overflow-hidden rounded-xl border border-border bg-card shadow-card-elegant transition-colors hover:border-primary/40"
      >
          ...
          <div className="mt-4 flex items-center justify-between border-t border-border/60 pt-3 text-xs text-muted-foreground">
            ...
            <div className="flex items-center gap-3">
              <span className="inline-flex items-center gap-1">
                <Heart className="h-3.5 w-3.5" /> {project.likes_count}
              </span>
              <span className="inline-flex items-center gap-1">
                <MessageCircle className="h-3.5 w-3.5" /> {project.comments_count ?? 0}
              </span>
            </div>
          </div>
        </div>
