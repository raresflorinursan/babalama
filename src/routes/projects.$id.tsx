...
function ProjectDetail() {
  ...
  const { data: liked } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await supabase.from("likes").select("id").eq("project_id", id).eq("user_id", user.id).maybeSingle();
      return !!data;
    },
  });
  ...
  const toggleLike = async () => {
    if (!user) { toast.error("Bitte einloggen, um zu liken"); return; }
    if (liked) {
  ...
  const submitComment = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!user) { toast.error("Bitte einloggen, um zu kommentieren"); return; }
    if (!comment.trim()) return;
    const { error } = await supabase.from("comments").insert({ project_id: id, user_id: user.id, comment: comment.trim() });
  ...
  if (isLoading) {
    return <SiteShell><div className="mx-auto max-w-5xl px-4 py-16">Lädt…</div></SiteShell>;
  }
  if (!project) {
    return (
      <SiteShell>
        <div className="mx-auto max-w-3xl px-4 py-24 text-center">
          ...
          <Button asChild className="mt-6"><Link to="/projects">Zur Galerie</Link></Button>
        </div>
      </SiteShell>
    );
  }

  const author = (project as any).profiles;
    ...
    <SiteShell>
      <div className="mx-auto max-w-5xl px-4 py-10 sm:px-6">
          ...
          <div className="p-6 sm:p-8">
            ...
            <div className="mt-5 flex flex-wrap items-center gap-2">
              ...
              <Button onClick={toggleLike} variant={liked ? "default" : "outline"} size="sm">
                <Heart className={"mr-2 h-4 w-4 " + (liked ? "fill-current" : "")} /> {project.likes_count}
              </Button>
            </div>
              ...
              <div className="md:col-span-2 space-y-6">
                ...
                {project.problem_solved && (
                  <Section title="Welches Problem löst es?">
                    <p className="whitespace-pre-line text-sm leading-relaxed text-muted-foreground">{project.problem_solved}</p>
                  </Section>
                )}
                {project.lessons_learned && (
                  <Section title="Was habe ich gelernt?">
                    <p className="whitespace-pre-line text-sm leading-relaxed text-muted-foreground">{project.lessons_learned}</p>
                  </Section>
                )}
              </div>
                ...
                <Section title="Tech-Stack">
                  <div className="flex flex-wrap gap-1.5">
                    {(project.technologies ?? []).map((t: string) => (
                      <span key={t} className="rounded-md bg-accent px-2 py-1 text-xs text-muted-foreground">{t}</span>
                    ))}
                  </div>
                </Section>
                <Section title="Autor">
                  {author ? (
                    <Link to="/profile/$id" params={{ id: author.id }} className="flex items-center gap-3 rounded-lg border border-border p-3 hover:bg-accent">
                      ...
                      <div>
                        <div className="text-sm font-medium">{author.username ?? author.full_name}</div>
        ...
        <section className="mt-10">
          <h2 className="flex items-center gap-2 text-lg font-medium"><MessageCircle className="h-5 w-5" /> Kommentare</h2>

          <form onSubmit={submitComment} className="mt-4 space-y-3">
            <Textarea
              value={comment}
              onChange={(e) => setComment(e.target.value)}
              placeholder={user ? "Schreibe einen Kommentar…" : "Bitte einloggen, um zu kommentieren"}
              disabled={!user}
              rows={3}
            />
            <Button type="submit" disabled={!user || !comment.trim()} className="bg-gradient-primary hover:opacity-90">
              Kommentar posten
            </Button>
          </form>
