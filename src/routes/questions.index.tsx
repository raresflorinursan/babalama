...
export const Route = createFileRoute("/questions/")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Stelle Coding- und KI-Fragen und hilf anderen, weiterzukommen.",
      },
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("questions")
        .select(
          "id, title, description, category, difficulty, status, created_at, user_id, profiles:user_id(username, full_name)",
        )
        ...
        <div className="mx-auto flex max-w-7xl flex-col gap-6 px-4 py-16 sm:px-6 md:flex-row md:items-end md:justify-between">
          ...
          <Button asChild className="bg-gradient-primary shadow-glow hover:opacity-90">
            <Link to="/ask-question">
              <Plus className="mr-2 h-4 w-4" /> Frage stellen
            </Link>
          </Button>
        </div>
      </section>
        ...
        {isLoading ? (
          ...
          <div className="rounded-xl border border-dashed border-border bg-card p-12 text-center">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Stelle die erste Frage und starte die Diskussion.
            </p>
            <Button asChild className="mt-5 bg-gradient-primary hover:opacity-90">
              <Link to="/ask-question">Frage stellen</Link>
            </Button>
          </div>
        ) : (
