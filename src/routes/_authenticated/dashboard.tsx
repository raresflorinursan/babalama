...
import { AlertCircle, Heart, MessageCircleQuestion, Plus, Upload, UserRound } from "lucide-react";
...
export const Route = createFileRoute("/_authenticated/dashboard")({
  head: () => ({
    meta: [
      { title: "Dashboard — Solvix" },
      { name: "description", content: "Verwalte deine Projekte und Fragen auf Solvix." },
    ],
  }),
  component: Dashboard,
});
...
function Dashboard() {
  ...
  const savedQuery = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("saved_projects")
        .select(
          "project_id, projects(id, title, slug, short_description, category, difficulty, technologies, image_url, likes_count)",
        )
        .eq("user_id", user!.id);
      if (error) throw error;
      return (data ?? [])
  ...
  const myQuestions = questionsQuery.data;
      ...
      <section className="mx-auto max-w-7xl space-y-12 px-4 py-12 sm:px-6">
        <Block title="Meine Projekte" count={myProjects?.length ?? 0}>
          {projectsQuery.isLoading ? (
            ...
            <LoadError onRetry={() => projectsQuery.refetch()} />
        ...
        <Block title="Gespeicherte Projekte" count={saved?.length ?? 0}>
          {savedQuery.isLoading ? (
            ...
            <LoadError onRetry={() => savedQuery.refetch()} />
        ...
        <Block title="Meine Fragen" count={myQuestions?.length ?? 0}>
          {questionsQuery.isLoading ? (
            ...
            <LoadError onRetry={() => questionsQuery.refetch()} />
...
function Empty({
  ...
  return (
    <div className="rounded-xl border border-dashed border-border bg-card p-10 text-center">
      <div className="mx-auto grid h-12 w-12 place-items-center rounded-full bg-accent text-primary-glow">
        {icon}
      </div>
...
function LoadingGrid() {
  return (
    <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3" aria-label="Inhalte werden geladen">
...
function LoadingRows() {
  return (
    <div className="space-y-2" aria-label="Fragen werden geladen">
      {Array.from({ length: 3 }, (_, index) => (
        <div
          key={index}
...
function LoadError({ onRetry }: { onRetry: () => void }) {
  return (
    <div className="flex flex-col items-center rounded-xl border border-destructive/30 bg-destructive/5 p-8 text-center">
      <AlertCircle className="h-6 w-6 text-destructive" />
      <p className="mt-3 text-sm">Inhalte konnten nicht geladen werden.</p>
